...
dat <- read.csv('BatCoV-assoc.csv', stringsAsFactors=FALSE)
...
getImpute <- function(listobject, m=10, rem=FALSE, useMeans=TRUE){
  ...
  no  <- which(colSums(is.na(htriz))/nrow(htriz) > 0.90)
	nop <- which(colSums(is.na(ptriz))/nrow(ptriz) > 0.90)
  if(length(no) > 0){
    htriz <- htriz[,-no]
  }
  if(length(nop) > 0){
...
# remove columns with correlation coeff > 0.7
  no2 <- which(cor(htriz, use='complete.obs')[,1] > 0.5)
  if(length(no2)>0){
    no2 <- no2[-1]
		htriz = htriz[,-no2]
  }
...
## interaction matrix
intMatFull <- table(
	sort(dat$host_species),
	sort(dat$virus_genus)
)
...
host$speciesName <- gsub('_', ' ', host$Pan)

# so there are some hosts whose interactions we know and traits we do not. Cool. 
h <- data.frame(speciesName=rownames(intMatFull))
h <- left_join(h, host, by='speciesName')
...
host2 <- data.frame(
	expandCol(h$MSW05_Order, 'hOrder'), 
...
virus <- dat %>%
	...
	summarise(nSubs=length(unique(virus_subgenus)))

virus2 <- data.frame(nSubs=virus$nSubs, dummy=1)
...
Imputation bonked, so now I just use mean values to fill in the NAs, which is far from ideal. 
...
ret <- linksDF[which(linksDF$virus == 'Betacoronavirus'), c(1,4)]

write.csv(ret, file='DallasPredictions.csv')
