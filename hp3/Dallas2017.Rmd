...
dat2 <- left_join(dat, virus[,c(1,5)], by='vVirusNameCorrected')

## interaction matrix
intMatFull <- table(sort(dat2$hHostNameFinal), 
	sort(dat2$vGenus))
...
virusG <- virus %>%
	group_by(vGenus) %>%
	summarise(species=length(unique(vVirusNameCorrected)), 
		ReverseZoonoseFrac=mean(ReverseZoonoses,na.rm=TRUE),
		IsHoSa=mean(IsHoSa,na.rm=TRUE), 
		vGenomeAveLength=mean(vGenomeAveLength, na.rm=TRUE), 
		sdvGenomeLength=sd(vGenomeAveLength, na.rm=TRUE), 
		vWOKcites=mean(vWOKcites, na.rm=TRUE), 
		vPubMedCites=mean(vPubMedCites, na.rm=TRUE), 
		vCytoReplicTF=mean(vCytoReplicTF, na.rm=TRUE),  
		vSegmentedTF=mean(vSegmentedTF,na.rm=TRUE), 
		vVectorYN=sum(vVectorYNna=='Y', na.rm=TRUE)/length(na.omit(vVectorYNna)), 
		vSS=sum(vSSoDS=='SS', na.rm=TRUE)/length(na.omit(vSSoDS)),
		vDNA=sum(vDNAoRNA=='DNA',na.rm=TRUE)/length(na.omit(vDNAoRNA)),
		vEnvelope=mean(vEnvelope=='enveloped', na.rm=TRUE), 
		fracZoonotic=mean(IsZoonotic, na.rm=TRUE))
virusG$vGenus <- NULL
...
## Points object
hvDat <- makePoints(intMatFull, host2, virusG)
...
linksCov <- filter(linksDF, virus=='Betacoronavirus')
linksCov2 <- filter(linksCov, presence==0)
linksCov3 <-linksCov2[order(linksCov2$suitability, decreasing=TRUE),]

top50 <- linksCov3[1:50,]
write.csv(top50, file='DallasMammalPredictions.csv')


#bat predictions

linksCovBat <- left_join(linksCov3, host, by=c('host'='hHostNameFinal'))
linksCovBat2 <- linksCovBat[which(linksCovBat$hOrder=='CHIROPTERA'), 1:5]
linksCovBat3 <-linksCovBat2[order(linksCovBat2$suitability, decreasing=TRUE),]

top50Bat <- linksCovBat3[1:50,]
write.csv(top50Bat, file='DallasBatPredictions.csv')
